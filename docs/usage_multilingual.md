...
- `按需缓存（512 MiB）`：仅获取使用到的外观，超过预算后清理未被引用的最久未使用资源。
- `永久保留已下载资源`：仅获取使用到的外观，并长期保留成功缓存的文件。
...
- `On-demand cache (512 MiB)`: fetch used appearances and evict the least-recently-used unreferenced files above the budget.
- `Keep downloaded resources`: fetch used appearances and retain successfully cached files.
...
- `オンデマンドキャッシュ（512 MiB）`：使用した外観だけを取得し、上限を超えると参照されていない古いファイルから整理します。
- `ダウンロード済みリソースを保持`：使用した外観だけを取得し、正常にキャッシュしたファイルを保持します。
- どちらの方式も 449 オペレーターの全リソースを事前ダウンロードしません。

## Alpha notes / Alpha 说明 / Alpha 注意事項

- 中文：已在 Oxygen Not Included `740622` 实测同一存档中的四个不同干员。动作转盘仅控制视觉表演，不改变工作、生命、压力、碰撞或模拟状态。发布包包含代码、轻量目录与第三方声明，外观资源按需获取。问题反馈请加入 QQ 群 `785437890` 或使用 GitHub Issues。
- English: Tested on Oxygen Not Included build `740622` with four different operators in one save. Action-wheel entries only control visual performances; jobs, health, stress, collision, and simulation state remain unchanged. The package contains code, lightweight catalog metadata, and third-party notices; appearance resources are retrieved on demand. Send feedback through QQ group `785437890` or GitHub Issues.
- 日本語：Oxygen Not Included `740622` で、1 つのセーブに 4 人の異なるオペレーターを割り当てて確認済みです。アクションホイールは表示上の演技だけを制御し、作業、生命、ストレス、当たり判定、シミュレーション状態は変更しません。配布パッケージにはコード、軽量カタログ、第三者表記が含まれ、外観リソースは必要時に取得します。フィードバックは QQ グループ `785437890` または GitHub Issues へお寄せください。
