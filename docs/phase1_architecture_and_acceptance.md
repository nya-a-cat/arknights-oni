...
当前证据状态：**可玩主路径已通过游戏内验收，第一阶段的长期缓存与压力项仍待补测。** 已通过零警告 DLL 编译、26 项动画映射断言、12 项外观目录断言、8 项缓存索引断言、四路冷缓存与调用者独立取消解析、同长度缓存损坏恢复和 Playwright 7/7；ONI 模组页 Options、中文名搜索、皮肤/模型联动、`Ctrl+F8`、四个复制人实时切换、脚底对齐、移动/爬梯与配置持久化均有画面和 `Player.log` 证据。离线首次失败、永久策略、20 个复制人压力和引用释放仍保持待验证。旧版 130 个大图 sheet 只保留为原型历史。
...
- GitHub、Steam Workshop 和其他公开分发等待权利审查。
...
| WEB-002A | 干员中文名搜索 | PRTS 轻量目录、阿米娅、能天使 | 目录不少于 400 条；“阿米娅”解析为 `char_002_amiya`；选择“能天使”回填 `char_103_angel` | 已验证当前目录快照 |
...
- Mod `0.3.1` 从本地安装目录成功加载，配置采用 `OnDemandCache`；从 Steam 启动时未触发 Mod Safe Mode。
